...
class AudioManager {
  ...
  playEffect(key: string, volume = 1.0): void {
    ...
    this.playingEffects.push({ source, gainNode });
  }
  ...
  set currentTime(value: number) {
    ...
    if (source.buffer) {
      ...
      this.playLoop(this.looping, this.musicVolume_);
      source.start(this.context.currentTime, seekTime);
    }
  }
  ...
  playMusic(key: string, volume = 1.0): void {
    if (this.looping) {
      this.stopLoop(this.looping);
    }
  
    this.looping = key;
    this.playLoop(key, volume);
  }
    
  playLoop(key: string, volume = 1.0): void {
